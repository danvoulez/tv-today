...
class DBAdapter:
    ...
    @property
    def result_selector(self):
        return self.policy.result_selector or "a[href]"

    @property
    def title_selector(self):
        return self.policy.title_selector or "h1, h2, .title"
    ...
    @property
    def login_email_selector(self):
        return self.policy.login_email_selector or "input[type='email'], input[name='username']"

    @property
    def login_password_selector(self):
        return self.policy.login_password_selector or "input[type='password']"

    @property
    def login_submit_selector(self):
        return self.policy.login_submit_selector or "button[type='submit']"

    @property
    def login_success_selector(self):
        return self.policy.login_success_selector or ".logged,.user-menu"


# Backwards-compatible adapters kept for tests and legacy callers.
...
class InternetArchiveAdapter(GenericVideoAdapter):
    def classify_retrieval(self, url, page_info) -> tuple[bool, str | None]:
        if url and "archive.org/download/" in url:
            return True, "official_download"
        return super().classify_retrieval(url, page_info)


class VimeoAdapter(GenericVideoAdapter):
    def classify_retrieval(self, url, page_info) -> tuple[bool, str | None]:
        if page_info.get("has_download_button"):
            return True, "official_download"
        return False, None
...
async def get_adapter_for_domain(domain: str, db: AsyncSession | None = None):
    if db is None:
        mapping = {
            "archive.org": InternetArchiveAdapter,
            "vimeo.com": VimeoAdapter,
        }
        adapter_cls = mapping.get(domain, GenericVideoAdapter)
        return adapter_cls()
